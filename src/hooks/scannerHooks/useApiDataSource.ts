...
export const useApiDataSource = (timeframe) => {
  ...
  const loadApiData = useCallback(async (
    ...
    console.log("loadApiData called with:", { 
      symbols: symbols.length, 
      forceRefresh, 
      runFullBacktest, 
      timeframe, 
      apiKey: apiKeyConfig.key ? `...${apiKeyConfig.key.slice(-4)}` : 'none',
    ...
    try {
      ...
      if (!forceRefresh) {
        try {
          ...
          console.log("Cache check results:", {
            patternsFound: cachedPatterns?.length || 0,
            backtestResultsFound: cachedBacktestResults?.length || 0,
            statsFound: !!cachedStats
          });
      ...
      try {
        ...
        console.log("Calling fetchPatternData with:", {
        ...
        console.log(`Received ${rawPatterns.length} raw patterns from API`);
      ...
      console.log("Successfully loaded and processed data from API:", {
        patternCount: patterns.length,
        backtestCount: backtestResults.length,
        cacheStatus: success ? 'Cached' : 'Updated',
        timeframe
      });
      
      // Cache the API results for future use
      await Promise.all([
        storeCachedPatterns(timeframe, patterns),
        storeCachedBacktestResults(timeframe, backtestResults),
        storeCachedStats(timeframe, stats)
      ]);
      ...
      console.log("API data loading completed with status:", { success, timeframe });
    }
