...
const SUPABASE_EDGE_FUNCTION_URL = import.meta.env.VITE_SUPABASE_EDGE_FUNCTION_URL || 'https://ttmeplqmrjhysyqzuaoh.supabase.co/functions/v1/polygon-data';

export class PolygonApiClient {
  private apiKey: string;
  private rateLimit: number = 5; // Requests per second for free tier
  private retryLimit: number = 3;
  private useSupabaseProxy: boolean = true; // Enable the Supabase proxy by default
  ...
  async checkApiKey(): Promise<{ isValid: boolean; isPremium: boolean }> {
    try {
      ...
      if (this.useSupabaseProxy) {
        url = `${SUPABASE_EDGE_FUNCTION_URL}?ticker=AAPL&action=reference`;
      } else {
        url = `https://api.polygon.io/v3/reference/tickers/AAPL?apiKey=${this.apiKey}`;
      }
  ...
  async getHistoricalPrices(
    ...
    try {
      ...
      if (this.useSupabaseProxy) {
        ...
        url = `${SUPABASE_EDGE_FUNCTION_URL}?ticker=${symbol}&timeframe=${multiplier}&timespan=${timespan}&from=${formattedStartDate}&to=${formattedEndDate}&limit=${limit}`;
      } else {
        // Use direct Polygon.io API
        url = `https://api.polygon.io/v2/aggs/ticker/${symbol}/range/${multiplier}/${timespan}/${formattedStartDate}/${formattedEndDate}?apiKey=${this.apiKey}&limit=${limit}`;
      }
  ...
  async getRealTimeQuote(symbol: string): Promise<any> {
    try {
      ...
      if (this.useSupabaseProxy) {
        url = `${SUPABASE_EDGE_FUNCTION_URL}?ticker=${symbol}&action=lasttrade`;
      } else {
        url = `https://api.polygon.io/v2/last/trade/${symbol}?apiKey=${this.apiKey}`;
      }
  ...
  async isMarketOpen(): Promise<boolean> {
    try {
      ...
      if (this.useSupabaseProxy) {
        url = `${SUPABASE_EDGE_FUNCTION_URL}?action=marketstatus`;
      } else {
        url = `https://api.polygon.io/v1/marketstatus/now?apiKey=${this.apiKey}`;
      }
  ...
  async getAggregates(
    ...
    try {
      ...
      if (this.useSupabaseProxy) {
        url = `${SUPABASE_EDGE_FUNCTION_URL}?ticker=${symbol}&timeframe=${multiplier}&timespan=${timespan}&from=${fromStr}&to=${toStr}`;
      } else {
        url = `https://api.polygon.io/v2/aggs/ticker/${symbol}/range/${multiplier}/${timespan}/${fromStr}/${toStr}?apiKey=${this.apiKey}`;
      }
  ...
  async getLatestTrades(symbol: string, limit: number = 10): Promise<any> {
    try {
      ...
      if (this.useSupabaseProxy) {
        url = `${SUPABASE_EDGE_FUNCTION_URL}?ticker=${symbol}&limit=${limit}&action=trades`;
      } else {
        url = `https://api.polygon.io/v3/trades/${symbol}?limit=${limit}&apiKey=${this.apiKey}`;
      }
